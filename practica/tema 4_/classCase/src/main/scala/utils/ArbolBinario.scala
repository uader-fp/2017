...
object ArbolBinario {
  ...
  def insert(nro: Int, tree: Tree): Tree = tree match {
    ...
    else if (nro < nroTree) Node(nroTree, insert(nro, left), right)
    else Node(nroTree, left, insert(nro, right))
  }
  ...
  def apply(tree: Tree, fx: (Int) => Int): Tree = tree match {
    ...
    case Node(nroTree, left, right) => Node(fx(nroTree), apply(left,fx),apply(right,fx))
  }

  /**
   * Devuelve un arbol con el doble de los elementos
   */
  def double(tree: Tree): Tree = apply(tree, (a:Int) => a*2)
  ...
  def toList(tree: Tree): List[Int] = tree match {
    case EmptyTree()                => List()
    case Node(nroTree, left, right) => toList(left)++(nroTree::toList(right))
  }
  ...
  def toString(tree: Tree): String = tree match {
    case EmptyTree()                => ""
    case Node(nroTree, left, right) => toString(left)+ " " + nroTree.toString()+ " " + toString(right)
  }
